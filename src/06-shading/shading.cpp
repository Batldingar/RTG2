...
void mouse_callback(GLFWwindow* window, double xpos, double ypos)
{
	...
	mousePos.x = (xpos / screenWidth) * 2.0f - 1.0f;  // Mapping range to [-1,1]
	mousePos.y = 1.0f - (ypos / screenHeight) * 2.0f;  // Y is flipped

	//std::cout << "Mouse Position (NDC): X = " << mousePos.x << ", Y = " << mousePos.y << std::endl;
}
